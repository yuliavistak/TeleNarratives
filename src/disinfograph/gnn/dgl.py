...
def build_dgl_dataset(
    ...
    for canonical_etype, rel_arr in relations.items():
        ...
        allowed_src = {old: new for new, old in allowed_src.items()}
        allowed_tgt = (
            nodes[tgt]
            .dropna()
            .reset_index()
        ...
        allowed_tgt = {old: new for new, old in allowed_tgt.items()}
    ...
    claim_labels = nodes["claim"][["label"]].map(numericalise_labels)
...
def save_dgl_graph(dgl_graph, path: Union[str, Path] = "mumin.dgl"):
    ...
    # Convert masks to unsigned 8-bit integers for every node type that has them.
    for mask_name in ["train_mask", "val_mask", "test_mask"]:
        for node_type in dgl_graph.ntypes:
            if mask_name not in dgl_graph.nodes[node_type].data:
                continue
            mask_tensor = dgl_graph.nodes[node_type].data[mask_name]
            dgl_graph.nodes[node_type].data[mask_name] = mask_tensor.type(torch.uint8)
...
def load_dgl_graph(path: Union[str, Path] = "mumin.dgl"):
    ...
    # Convert masks back to booleans for every node type that has them.
    for mask_name in ["train_mask", "val_mask", "test_mask"]:
        for node_type in dgl_graph.ntypes:
            if mask_name not in dgl_graph.nodes[node_type].data:
                continue
